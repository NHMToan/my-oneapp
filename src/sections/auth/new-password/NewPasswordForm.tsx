...
export default function NewPasswordForm() {
  ...
  const [onChangePassword] = useChangePasswordMutation();
  ...
  const handlePaste = (event) => {
    ...
    [].forEach.call(document.querySelectorAll(".field-code"), (node, index) => {
      ...
      const fieldIndex: any = `code${index + 1}`;
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const res = await onChangePassword({
        variables: { code, newPassword: data.password },
      });
      ...
      if (res?.data?.changePassword?.success) {
        ...
        throw res.data?.changePassword?.message || "Server error";
      }
    } catch (error) {
      console.error(error);
    }
