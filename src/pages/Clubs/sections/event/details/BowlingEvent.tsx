...
interface EventDetailsBowlingEventProps {
  event: any;
  onClose: () => void;
  onRefreshList?: () => void;
  refetchStats?: () => void;
...
const BowlingEvent: FC<EventDetailsBowlingEventProps> = ({
  event,
  loading,
  onRefreshList,
  onClose,
  ...
  const { title, status, isAdmin } = event;

  const renderGeneral = () => {
    const { description, start, end, address, time, maxVote, price } = event;
    return (
      <Card>
  ...
  const TABS = [
    {
      ...
      component: (
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} md={6}>
            <EventCard event={event} hideInfo />
          </Grid>
        </Grid>
      ),
    },
    {
      ...
      component: (
        <EventDetailsVotesTab event={event} refetchStats={refetchStats} />
      ),
    },
    {
      title: "History",
      ...
      component: <EventHistory event={event} />,
    },
  ];
                ...
                try {
                  if (status === 1) {
                    const res = await onChangeStatus({
                      variables: { id: event.id, status: 2 },
                    });
                  ...
                  if (status === 2) {
                    const res = await onChangeStatus({
                      variables: { id: event.id, status: 1 },
                    });
                    if (res.data.changeEventStatus.success) {
                      enqueueSnackbar(
                        translate("club.event.form.update_success")
